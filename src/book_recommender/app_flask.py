'''Flask Full Stack website
Frontend is built using HTML and BootStrap
The website is hosted on AWS and used DNS hostname
The core function of the website is protected by password

'''
from typing import Any
from flask import Flask, render_template, request, redirect, url_for

from .model import custom_recommender, item_based_recommender, content_based_recommender
...
def verify_password(username: Any, password: Any):
    '''This function is verifing the password
...
def homepage():
    '''This function is for the homepage
    ...
    if request.method == 'POST':
        ...
        if model == 'Cosine_sim':
            ...
            for i in range(len(recom_data['book_title'].tolist())):
                url = books.loc[books['book_title'] == recom_data['book_title'].tolist()[
                    i], 'img_l'][:1].values[0]
                title = books.loc[books['book_title'] == recom_data['book_title'].tolist()[
                    i], 'book_title'][:1].values[0]
                links.append(url)
                titles.append(title)
